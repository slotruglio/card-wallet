from typing import Type, TypeVar, Any
from pydantic import BaseModel
from sqlalchemy.orm import DeclarativeBase
import uuid

ORM = TypeVar("ORM", bound=DeclarativeBase)
Pyd = TypeVar("Pyd", bound=BaseModel)


class OrmPydanticHelper:
    @staticmethod
    def orm_to_pydantic(orm_instance: ORM, pyd_model: Type[Pyd], nested: bool = True) -> Pyd:
        """
        Convert ORM -> Pydantic
        If nested=True, also converts relationships if they are Pydantic-compatible
        """
        data = {}
        for field in pyd_model.__fields__:
            if hasattr(orm_instance, field):
                value = getattr(orm_instance, field)
                # convert UUID to str
                if isinstance(value, uuid.UUID):
                    value = str(value)
                # recursively convert nested ORM
                elif nested and hasattr(value, "__iter__") and not isinstance(value, (str, bytes)):
                    # handle lists of related objects
                    value = [OrmPydanticHelper.orm_to_pydantic(v, pyd_model.__fields__[field].type_) for v in value]
                elif nested and hasattr(value, "__table__"):
                    # single related object
                    value = OrmPydanticHelper.orm_to_pydantic(value, pyd_model.__fields__[field].type_)
                data[field] = value
        return pyd_model(**data)

    @staticmethod
    def pydantic_to_orm(pyd_instance: Pyd, orm_model: Type[ORM]) -> ORM:
        """
        Convert Pydantic -> ORM
        Handles UUID conversion automatically
        """
        data = pyd_instance.dict(exclude_unset=True)
        if "id" in data and isinstance(data["id"], str):
            try:
                data["id"] = uuid.UUID(data["id"])
            except ValueError:
                pass  # keep original if not a valid UUID
        return orm_model(**data)
